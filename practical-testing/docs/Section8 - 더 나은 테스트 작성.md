...
> private 메소드 테스트는 할 필요가 없다. 하려고 해서는 안된다.

이유가 뭘까? <br>

어떠한 객체에 public 메소드를 가지고 있다는 건 외부(Controller) 에서 봤을 때는 외부로 노출되지 않은 내부 기능까지 알 필요가 없다 <br>
외부는 public 메소드을 기능이 잘 동작하는지만 알면 된다 <br>

어차피 public 메소드를 테스트를 할때 자동으로 같이 검증이 된다 <br>

만약 private 의 테스트를 해야할 것 같다면 위 질문을 던져보자 <br>
> 🖐🏻 객체를 분리할 시점인가? 

✅ private 메소드의 책임이 분리가 되야할 것 같다는 생각이 강하게 들면 클래스를 분리해서 책임을 분리를 한다 <br>
그리고 그 public 메소드를 테스트를 해야한다 <br>

좋은 설계를 하기 위해 꾸준히 노력을 해보자.

### 11) 테스트에서 필요한 메소드가 있는대, 프로덕션 코드에는 필요가 없다면?
> 프로덕션 코드에 테스트 전용 코드를 만들어도 된다. 하지만 '보수적' 으로 접근할 필요가 있다.


### 12) 키워드 정리
1) 테스트 하나 당 목적은 1개 -> 한 문단에는 1개의 주제! 분기문이나 반복문 등 추가적인 고민을 요하는 로직들이 들어가서는 안된다.
   - 고민을 요하는 테스트는 테스트 케이스가 여러가지 라는 반증하기 때문에 별도의 테스트 케이스로 분리를 해야한다.
2) 완벽한 제어
3) 테스트 환경의 독립성
4) Test Fixture
5) deleteAll(), deleteAllInBatch() -> 데이터 클렌징
6) @ParameterizedTest, @DynamicTest
7) 수행환경 통합하기 -> 테스트 수행 자체가 비용이다. 공통 환경을 만들어서 사용해보자 -> 서버가 뜨는 시간을 줄이자.
8) private method test -> 수행할 필요가 없다.
9) 테스트에서만 필요한 코드 -> 만들어도 좋지만, 보수적으로 접근하자